#!/usr/bin/env bash
# IITD Repository Setup Module — apt mirror actions (backups moved to Backups menu)

MODULE_ID="iitd_repo"
MODULE_NAME="IITD Repository Setup"
MODULE_DESCRIPTION="Configure IITD apt mirror and disable extra repos"
MODULE_ORDER=10
...
show_repo_submenu() {
    ...
    echo "  1) Apply IITD mirror (sources.list)"
    echo "  2) Disable ${official_src}"
    echo "  3) Disable 3rd party repositories"
    echo "  4) Run apt update"
    echo
    echo "  Tip: Backup / Restore → main menu → Backups & Restore"
    echo
